import { combineRgb } from '@companion-module/base';

export function initFeedbacks() {
	let self = this;
	let feedbacks = {};

	const foregroundColor = combineRgb(255, 255, 255) // White
	const backgroundColorRed = combineRgb(255, 0, 0) // Red

	const  gradientPNG = ''

	feedbacks.powerState = {
		type: 'boolean',
		name: 'Last Power State',
		description: 'Indicate if Device is On or Off based on last known power state',
		defaultStyle: {
			color: foregroundColor,
			bgcolor: '#330000',
		},
		options: [
			{
				type: 'dropdown',
				label: 'Indicate in X State',
				id: 'option',
				default: 'on',
				choices: [
					{ id: 'off', label: 'Off' },
					{ id: 'on', label: 'On' },
				],
			},
		],
		callback: function (feedback) {
			if (self.INFO && self.INFO.power) {
				if (self.INFO.power === feedback.options.option) {
					return true;
				}
			}
			return false
		}
	},
	feedbacks.gradientState = {
		type: 'boolean',
		name: 'Gradient State',
		description: 'Indicate if the Gradient Toggle is On or Off',
		defaultStyle: {
			color: foregroundColor,
			bgcolor: backgroundColorRed,
			png64: gradientPNG,
		},
		options: [
			{
				type: 'dropdown',
				label: 'Indicate in X State',
				id: 'option',
				default: 'on',
				choices: [
					{ id: 'off', label: 'Off' },
					{ id: 'on', label: 'On' },
				],
			},
		],
		callback: function (feedback) {
			let opt = feedback.options.option === 'on' ? true : false
			if (self.INFO && self.INFO.gradienttoggle) {
				if (self.INFO.gradienttoggle === opt) {
					return true;
				}
			}
			return false
		}
	},
	// feedbacks.currentColor = {
	// 	type: 'boolean',
	// 	name: 'Last Power State',
	// 	description: 'Indicate if Device is On or Off based on last known power state',
	// 	defaultStyle: {
	// 		color: foregroundColor,
	// 		bgcolor: backgroundColorRed,
	// 	},
	// 	options: [
	// 		{
	// 			type: 'dropdown',
	// 			label: 'Indicate in X State',
	// 			id: 'option',
	// 			default: 'on',
	// 			choices: [
	// 				{ id: 'off', label: 'Off' },
	// 				{ id: 'on', label: 'On' },
	// 			],
	// 		},
	// 	],
	// 	callback: function (feedback) {
	// 		if (self.INFO && self.INFO.power) {
	// 			if (self.INFO.power === feedback.options.option) {
	// 				return true;
	// 			}
	// 		}
	// 		return false
	// 	}
	// }

	self.feedbackIds = Object.keys(feedbacks);
	self.setFeedbackDefinitions(feedbacks);
}

export function checkAllFeedbacks() {
	if (this.feedbackIds?.length) {
		this.checkFeedbacks(...this.feedbackIds)
	}
}